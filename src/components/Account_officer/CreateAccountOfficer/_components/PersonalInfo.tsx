...
const PersonalInfo = ({ control, setValue, trigger, watch }: PersonalInfoProps) => {
    ...
    const { data: singleACcountant, isLoading } = useGetSingleUserQuery(userId)


    useEffect(() => {
        if (singleACcountant?.data) {
            Object.entries(singleACcountant.data).forEach(([key, value]) => {
                if (value) setValue(key, value); // Only update non-empty values
            });
        }
    }, [singleACcountant, setValue]);
            ...
            <div className="border rounded-md">
                ...
                <div className="m-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    <Controller
                        ...
                        defaultValue={singleACcountant?.data?.firstName || ""}
                        render={({ field, fieldState: { error } }) => (
                            <div>
                    ...
                    <Controller
                        ...
                        defaultValue={singleACcountant?.data?.lastName || ""}
                        render={({ field, fieldState: { error } }) => (
                            <div>
                    ...
                    <Controller
                        ...
                        defaultValue={singleACcountant?.data?.email || ""}
                        render={({ field, fieldState: { error } }) => (
                            <div>
                                ...
                                <Input
                                    ...
                                    readOnly={singleACcountant?.data?.email ? true : false}
                                />
                                {error && <p className="text-red-500 text-sm">{error.message}</p>}
                            </div>
                        )}
